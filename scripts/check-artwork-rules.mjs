...
if (!artworkSource.includes("const ART_STORAGE_PREFIX = 'gdl_artwork16_';")) {
	throw new Error('Artwork storage prefix must be current and versioned.');
}
if (!artworkSource.includes("'gdl_artwork15_'")) {
	throw new Error('Previous artwork storage prefix history must include gdl_artwork15_.');
}
...
if (!artworkSource.includes('const shortcutIconInFlight = new Map<string, Promise<boolean>>()')
	...
	|| !unlinkingSource.includes('supersedeArtworkApplications(shortcutAppId, true)')
...
// 9. Achievement resolver must include all metadata achievements so total is not truncated to state-only count
