...
try {
  ...
  if (/const\s+nameApplied\s*=\s*false[\s\S]{0,500}const\s+nameReady\s*=\s*true/.test(linking)) {
	...
	if (!libraryArtworkSource.includes('retiredCommunityArtworkPreferred(steamAppId)')
		|| portraitSourceBlock.indexOf('preferredCommunity?.portrait') > portraitSourceBlock.indexOf('modern?.portrait')) {
		fail(libraryArtwork, 'retired games must prefer SteamGridDB Highest Score artwork over Steam-generated legacy canvases');
	}
	const heroSourceBlock = libraryArtworkSource.match(/urls:\s*\[[\s\S]*?imageType:\s*1,\s*label:\s*['"]Hero['"]/i)?.[0] || '';
	if (!heroSourceBlock || heroSourceBlock.includes('/header.jpg')) {
