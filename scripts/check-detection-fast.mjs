...
// --------------------------------------------------------------------------
// DFAST16: Bulk policy safety: rejects uncorroborated partial candidates
// --------------------------------------------------------------------------
{
	const dummyContext = {
		shortcutAppId: 1001,
	...
	if (result.safe) {
		throw new Error(`DFAST16 Failed: Bulk policy accepted uncorroborated partial candidate. Result: ${JSON.stringify(result)}`);
	}
	console.log('✓ DFAST16 Passed: Bulk linking strictly rejects uncorroborated partial candidates.');
}
