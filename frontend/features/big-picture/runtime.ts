...
const LIBRARY_TAB_SELECTOR = '[role="tab"], [class*="TabItem"], [class*="tabItem"], [class*="LibraryTab"], [class*="FilterTab"], [class*="tab_"], [class*="Tab_"], [class*="TopBar"] button, [class*="TabBar"] button';

const lastActiveTabEdge = new WeakMap<Document, 'first' | 'last' | 'middle'>();

function isElementSelectedTab(el: HTMLElement): boolean {
...
function trackVisibleTabEdges(doc: Document): void {
...
function pollLibraryTabLoop(doc: Document): void {
	if (!gdlBigPictureActive || !doc.body?.isConnected) {
		...
		if (now - lastLibraryTabNavTime > 220) {
			...
			if (visibleTabs.length > 1) {
				...
				if (btnRB && !prevRB && activeIndex === visibleTabs.length - 1) {
					// User pressed RB on the last visible tab (e.g. "INSTALADOS"): wrap to first tab!
					...
					// User pressed LB on the first visible tab (e.g. "COMPATIBLE"): wrap to last tab!
					lastLibraryTabNavTime = now;
					lastActiveTabEdge.set(doc, 'last');
