						...
						<div key={group.dateLabel} className="gdl-bp-feed-group">
							...
							<div className="gdl-bp-feed-list">
								{group.items.map((entry, idx) => {
									...
									const isPatch = eventType === 0 && !item.feedlabel && isPatchNoteItem(item);
									const label = eventType > 0 ? eventTypeLabel(eventType) : (item.feedlabel || (isPatch ? loc('AppActivity_MinorUpdate', 'ACTUALIZACIÓN MENOR / NOTAS DE PARCHE') : gdlText('feed_news', 'NOTICIAS')));
