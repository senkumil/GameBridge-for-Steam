...
export async function getCommunityContent(steamAppId: string): Promise<CommunityContentItem[]> {
	const language = await getSteamLanguage().catch(() => steamLanguageSync() || 'english');
	const cacheKey = `community6_${language}_${steamAppId}`;
...
export async function getNews(steamAppId: string): Promise<NewsItem[]> {
	...
	const preferredLanguage = await getSteamLanguage().catch(() => steamLanguageSync() || 'english');
	const cacheKey = `events8_${preferredLanguage}-en_${steamAppId}`;
	const cached = cacheGet<NewsItem[]>(cacheKey, CACHE_TTL.news);
	if (cached) return cached;
	...
	try {
		const [preferredJson, englishJson, announcementsJson, englishAnnouncementsJson] = await Promise.all([
			fetchPartnerEventsBackend({ steam_app_id: steamAppId, language: preferredLanguage }).catch(() => '{"items":[]}'),
			preferredLanguage === 'english'
				? Promise.resolve('{"items":[]}')
				: fetchPartnerEventsBackend({ steam_app_id: steamAppId, language: 'english' }).catch(() => '{"items":[]}'),
			fetchNewsBackend({ steam_app_id: steamAppId, language: preferredLanguage }).catch(() => '{"items":[]}'),
			preferredLanguage === 'english'
				? Promise.resolve('{"items":[]}')
				: fetchNewsBackend({ steam_app_id: steamAppId, language: 'english' }).catch(() => '{"items":[]}'),
		]);
		const preferred = JSON.parse(preferredJson);
		const english = JSON.parse(englishJson);
		const announcements = JSON.parse(announcementsJson);
		const englishAnnouncements = JSON.parse(englishAnnouncementsJson);
		const partnerItems = [
		...
		if (partnerItems.length > 0 || officialAnnouncements.length > 0) {
			...
			const merged = mergeSupplementalPatchNotes(steamAppId, deduped);
	...
	return mergeSupplementalPatchNotes(steamAppId, []);
}
