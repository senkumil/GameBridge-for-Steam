	...
	const handleReset = async (): Promise<void> => {
		...
		try {
			const result = await Promise.race([
				performFactoryReset({ deletePlaytime }),
				new Promise<{ ok: false; error: string }>(resolve => setTimeout(() => resolve({ ok: false, error: 'reset_timeout' }), 22_000)),
			]);
			if (result.ok) {
				setShowModal(false);
				...
				setShowModal(false);
				setStatus({
					text: gdlText('factory_reset_failed', 'Failed to reset plugin to factory state.'),
					color: '#d94126',
				});
			}
		} catch (err) {
